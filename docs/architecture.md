...
| `analytics` | `sprint_burndown()`/`sprint_burndown_summary()`/`sprint_capacity_daily()`/`effort_estimates()`/`sprint_effort_burndown()`/`sprint_velocity()`/`capacity_vs_velocity()`/`cycle_time_percentiles()`/`scope_change()` — compute metrics from stored data | `SprintMetricsRepository` | `DuckDbSprintMetricsRepository` (all the SQL lives here) |
