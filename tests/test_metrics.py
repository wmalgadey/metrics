...
def test_burndown_ideal_line_decreases_linearly_to_zero(conn):
...
def test_burndown_ideal_line_flat_over_weekend(conn):
    ...
    points = sprint_burndown(conn, "Proj\\Sprint 2")
    by_day = {p.day: p for p in points}
...
def test_burndown_excludes_removed_items_from_scope(conn):
    path = _seed_sprint(conn)
    snapshots = [
        {
...
def test_velocity_planned_vs_completed_and_rolling_average(conn):
    ...
    points = sprint_velocity(conn, ["Proj\\Sprint A", "Proj\\Sprint B"], rolling_window=2)
...
def test_capacity_vs_velocity_normalizes_by_hours(conn):
    path = _seed_sprint(conn)
    ...
    points = capacity_vs_velocity(conn, [path])
    assert len(points) == 1
...
def test_cycle_time_percentiles(conn):
    path = _seed_sprint(conn)
    items = [
        {
    ...
    result = cycle_time_percentiles(conn, [path])
    assert len(result) == 1
    r = result[0]
    assert r.count == 5
